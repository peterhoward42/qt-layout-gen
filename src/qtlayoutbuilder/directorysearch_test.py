...
class TestDirectorySearch(TestCase):
    ...
    def test_finds_all_files_expected(self):
        files, err = find_all_files(_TOP_LEVEL_DIR)
    ...
    def test_treats_empty_result_as_error(self):
        files, err = find_all_files(_EMPTY_DIR)
    ...
    def test_reports_os_level_error_properly(self):
        files, err = find_all_files(_NO_SUCH_DIR)
        self.assertIsNone(files)
        self.assertIsNotNone(err)
        msg = err.format_as_single_string()
        self.assertTrue('Problem in find_all_files for your path: <ridiculousdirname>' in msg)
        self.assertTrue("The system cannot find the path specified" in msg)
